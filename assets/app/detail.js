// Datamodel initalisation with empty object
var doc = {};
...
function loadText(id) {
  ...
  function showResult(doc) {
    ...
    const render = (pageNumber) => {
      ...
      // xml
      let namespaceRegex = /\s*xmlns="http:\/\/www\.tei-c\.org\/ns\/1\.0"\s*/gi; // Hack to remove noise generated by parser
      let xml = page.xml.toString().replace(namespaceRegex, " ");
      left.xml.innerHTML = "<textarea>" + xml + `</textarea>`;
      right.xml.innerHTML = "<textarea>" + xml + "</textarea>";
      // persons
      let personsHTML = "";
