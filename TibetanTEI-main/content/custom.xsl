...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
    ...
    <xsl:template match="tei:date[@when]">
        ...
        <!--<a style="color:p;font-weight:bold;text-decoration:none;" title="greg.: {@when}"> -->
        <a style="color:pink;text-decoration:none;" title="greg.: {@when}">
            <xsl:apply-templates/>
        </a>
    </xsl:template>
    ...
    <xsl:template match="tei:persName[@type = 'BUDA']">
        ...
        <a style="color:blue;text-decoration:none;" href="http://purl.bdrc.io/resource/{@key}"
            title="{@type} person ID: {@key}">
            <xsl:apply-templates/>
        </a>
    </xsl:template>
    ...
    <xsl:template match="tei:placeName[@type = 'BUDA']">
        <xsl:apply-templates select="@key"/>
        <a style="color:orange;text-decoration:none;" href="http://purl.bdrc.io/resource/{@key}"
            title="{@type} place ID: {@key}">
            <xsl:apply-templates/>
        </a>
    </xsl:template>
    ...
    <xsl:template match="tei:title[@type = 'RKTS']">
        ...
        <a style="color:green;text-decoration:none;"
            href="https://www.istb.univie.ac.at/kanjur/rktsneu/verif/verif3.php?id={@key}"
            title="{@type} text ID: {@key}">
            <xsl:apply-templates/>
        </a>
    </xsl:template>
    ...
    <xsl:template match="tei:persName[@type = 'SRC']">
        <xsl:apply-templates select="@key"/>
        <a style="color:blue;text-decoration:none;" href="https://sakyaresearch.org/persons/{@key}"
            title="{@type} person ID: {@key}">
            <xsl:apply-templates/>
        </a>
    </xsl:template>
    ...
    <xsl:template match="tei:placeName[@type = 'SRC']">
        <xsl:apply-templates select="@key"/>
        <a style="color:orange;text-decoration:none;" href="https://sakyaresearch.org/places/{@key}"
            title="{@type} place ID: {@key}">
            <xsl:apply-templates/>
        </a>
    </xsl:template>
    ...
    <xsl:template match="tei:title[@type = 'SRC']">
        <xsl:apply-templates select="@key"/>
        <a style="color:green;text-decoration:none;" href="https://sakyaresearch.org/sources/{@key}"
            title="{@type} source ID: {@key}">
            <xsl:apply-templates/>
        </a>
    </xsl:template>
